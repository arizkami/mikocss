from typography.base import (font_families, font_weights, letter_spacing,
                             line_heights, text_sizes)
...
def generate_typography_hpp(framework="gtk"):
    ...
    for size_name, size_data in text_sizes.items():
        font_size_px = (
            float(size_data["font_size"].replace("rem", "")) * 16
        )  # Convert rem to px (assuming 16px base)
        line_height_px = (
            float(size_data["line_height"].replace("rem", "")) * 16
            if "rem" in size_data["line_height"]
            else float(size_data["line_height"]) * font_size_px
        )
    ...
    for weight_name, weight_value in font_weights.items():
        ...
        lines.append(
            f'static constexpr FontWeight {var_name} = {{{weight_value}, "{weight_name}"}};'
        )
...
def generate_typography_wrapper_hpp(framework="gtk"):
    ...
    if framework == "gtk":
        ...
        if framework == "qt5":
            lines.append(
                "    font.setWeight(static_cast<QFont::Weight>(font_weight.weight));"
            )
        else:  # qt6
        ...
        lines.append(
            'inline HFONT create_font(const std::string& size, const std::string& weight, const std::string& family = "sans") {'
        )
        lines.append("    auto text_size = get_text_size(size);")
        lines.append("    auto font_weight = get_font_weight(weight);")
        lines.append("    ")
