from layout.base import (aspect_ratios, box_decoration_break, box_sizing,
                         break_values, clear_values, columns, display,
                         float_values, inset_values, isolation, object_fit,
                         object_position, overflow, overscroll_behavior,
                         position, visibility, z_index)
...
def generate_layout_hpp(framework="gtk"):
    ...
    for name, value in aspect_ratios.items():
        ...
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "aspect-ratio"}};'
        )
    lines.append("")
    ...
    for name, value in columns.items():
        ...
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "columns"}};'
        )
    lines.append("")
    ...
    for name, value in display.items():
        ...
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "display"}};'
        )
    lines.append("")
    ...
    for name, value in position.items():
        ...
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "position"}};'
        )
    lines.append("")
    ...
    for name, value in overflow.items():
        ...
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "overflow"}};'
        )
    lines.append("")
    ...
    for name, value in z_index.items():
        var_name = f"z_{name}"
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "z-index"}};'
        )
    lines.append("")
    ...
    for name, value in inset_values.items():
        ...
        lines.append(
            f'static constexpr LayoutProperty {var_name} = {{"{name}", "{value}", "inset"}};'
        )
    lines.append("")
...
def generate_layout_wrapper_hpp(framework="gtk"):
    ...
    for func_name, values_dict, prefix in categories:
        lines.append(
            f"inline LayoutProperty get_{func_name}(const std::string& name) {{"
        )
        lines.append(
            f"    static const std::unordered_map<std::string, LayoutProperty> {func_name}_map = {{"
        )
        ...
        lines.append(
            f"    return (it != {func_name}_map.end()) ? it->second : {default_var};"
        )
        lines.append("}\n")
    ...
    if framework == "gtk":
        lines.append(
            "inline void apply_layout_style(GtkWidget* widget, const std::string& css_class) {"
        )
        lines.append(
        ...
        lines.append("")
        lines.append(
            "inline void set_widget_position(GtkWidget* widget, const std::string& pos_type) {"
        )
        ...
        lines.append(
            "inline void apply_layout_style(QWidget* widget, const std::string& property, const std::string& value) {"
        )
        lines.append("    QString styleSheet = widget->styleSheet();")
        lines.append(
            '    styleSheet += QString::fromStdString(property + ": " + value + ";");'
        )
        lines.append("    widget->setStyleSheet(styleSheet);")
        lines.append("}")
        lines.append("")
        lines.append(
            "inline void set_widget_display(QWidget* widget, const std::string& display_type) {"
        )
        ...
        lines.append(
            "inline void set_window_position(HWND hwnd, const std::string& pos_type, int x, int y, int width, int height) {"
        )
        lines.append("    auto pos = get_position(pos_type);")
        ...
        lines.append(
            "        SetWindowPos(hwnd, NULL, rect.left + x, rect.top + y, width, height, flags);"
        )
        lines.append("    }")
        lines.append("}")
        lines.append("")
        lines.append(
            "inline void set_window_visibility(HWND hwnd, const std::string& visibility_type) {"
        )
