def generate_color_hpp(all_colors):
    ...
    for name, shades in all_colors.items():
        ...
        for shade_key, (r, g, b) in shades.items():
            ...
            lines.append(f"static constexpr Color {var_name} = {{ {r:.6f}f, {g:.6f}f, {b:.6f}f, 1.0f }};")
            total_colors += 1
...
def generate_wrapper_hpp(all_colors, framework="gtk"):
    ...
    lines.append("    static const std::unordered_map<std::string, Color> color_map = {")
    ...
    if framework == "gtk":
        ...
        lines.append("    auto c = get_color(name);")
        lines.append("    return RGB(static_cast<int>(c.r * 255), static_cast<int>(c.g * 255), static_cast<int>(c.b * 255));")
        lines.append("}")
        lines.append("")
...
def generate_color_list(all_colors):
    ...
    lines = []
    lines.append(f"// Total colors available: {sum(len(shades) for shades in all_colors.values())}")
    ...
    for color_name, shades in sorted(all_colors.items()):
        ...
        # Group by type
        standard = [k for k in shades.keys() if isinstance(k, int) and k in [50, 100, 200, 300, 400, 500, 600, 700, 800, 900]]
        ...
        if standard:
            shade_list = ", ".join([f"{color_name}-{shade}" for shade in sorted(standard)])
        ...
        if variants:
            variant_list = ", ".join([f"{color_name}-{variant}" for variant in sorted(variants)])
