...
from colors.generators import (
    generate_shades,
    generate_extended_shades,
    generate_saturation_variants,
    generate_temperature_variants
)
from generators.cpp import generate_color_hpp, generate_wrapper_hpp, generate_color_list
from generators.typography import generate_typography_hpp, generate_typography_wrapper_hpp
...
class ColorProcessor:
    ...
    def generate_cpp_files(self, output_dir="."):
        ...
        with open(f"{output_dir}/miko_typography_wrapper.hpp", "w", encoding="utf-8") as f:
    ...
    def get_color_stats(self):
        ...
        for name, shades in self.all_colors.items():
            standard = len([k for k in shades.keys() if isinstance(k, int) and k in [50, 100, 200, 300, 400, 500, 600, 700, 800, 900]])
            micro = len([k for k in shades.keys() if isinstance(k, int) and k not in [50, 100, 200, 300, 400, 500, 600, 700, 800, 900]])
