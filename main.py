...
"""
    ...
    if framework == "gtk":
        ...
        cmake_content += (
            "target_link_libraries(example Qt6::Core Qt6::Widgets Qt6::Gui)\n"
        )
...
def main():
    ...
    init_parser = subparsers.add_parser(
        "init", help="Initialize C++ project with styling files"
    )
    ...
    if args.cppframework:
        ...
        if args.cppframework == "gtk":
            ...
            print(
                f'   miko::set_window_position(hwnd, "fixed", 100, 100, 800, 600) // Win32'
            )

        print(f"\n📏 Available utilities:")
        print(
            f"   • Text sizes: xs, sm, base, lg, xl, 2xl, 3xl, 4xl, 5xl, 6xl, 7xl, 8xl, 9xl"
        )
        print(
            f"   • Font weights: thin, extralight, light, semilight, regular, medium, semibold, bold, extrabold, black, blackrr"
        )
        print(f"   • Display: block, inline, flex, grid, table, hidden, etc.")
        print(f"   • Position: static, relative, absolute, fixed, sticky")
        print(
            f"   • Aspect ratios: square, video, photo, portrait, wide, ultrawide, golden"
        )
